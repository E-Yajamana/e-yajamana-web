...
class AddForeignKeysToTbReservasiTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('tb_reservasi', function (Blueprint $table) {
            $table->foreign(['id_relasi'], 'tb_reservasi_ibfk_6')->references(['id_sulinggih'])->on('tb_sulinggih');
            $table->foreign(['id_upacaraku'], 'tb_reservasi_ibfk_7')->references(['id_upacaraku'])->on('tb_upacaraku');
        });
    }
